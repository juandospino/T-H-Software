...
class SensorMonitorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Monitor de Sensores DHT22 - Multiunidades")
    ...
    def setup_ui(self):
        ...
        # Título
        title_label = ttk.Label(main_frame, text="🌡️ Monitor de Sensores DHT22 - Multiunidades", 
                               font=("Arial", 16, "bold"))
        title_label.grid(row=0, column=0, columnspan=4, pady=(0, 15))
        
        # Frame de controles principales
        control_frame = ttk.LabelFrame(main_frame, text="Configuración Principal", padding="10")
        ...
        unit_frame = ttk.LabelFrame(main_frame, text="Unidades de Temperatura", padding="10")
        ...
        ttk.Button(button_frame, text="💾 Guardar Datos", 
                  command=self.save_data, width=15).grid(row=0, column=3, padx=5)
        
        ttk.Button(button_frame, text="📊 Ver Reporte", 
                  command=self.show_report, width=15).grid(row=0, column=4, padx=5)
        
        ttk.Button(button_frame, text="🔄 Reiniciar Gráficos", 
                  command=self.reset_graphs, width=15).grid(row=0, column=5, padx=5)
    ...
    def setup_plots(self):
        """Configurar los gráficos matplotlib"""
        ...
        # Colores y estilos
        self.colors = ['#1f77b4', '#2ca02c', '#d62728', '#9467bd']  # Colores más modernos
        self.lines_temp = []
        self.lines_hum = []
    ...
    def update_units(self):
        """Actualizar las unidades en la interfaz"""
    ...
    def convert_existing_data(self):
        """Convertir datos existentes a la unidad seleccionada"""
        if messagebox.askyesno("Convertir Datos", 
            ...
            # Recalcular todas las temperaturas convertidas
            unit = self.temp_unit_var.get()
            for i in range(4):
                sensor_key = f'sensor{i}'
                temp_c_list = list(self.data[sensor_key]['temp_c'])
                
                # Limpiar y recalcular temperaturas convertidas
    ...
    def show_report(self):
        ...
        try:
            ...
            report_text += f"🌡️ Unidad actual: {self.temp_unit_var.get().title()}\n"
            ...
            for sensor_idx in range(4):
                ...
                if valid_temps_c:
                    ...
                    if valid_temps_c:
                        ...
                        sensor_report += "🌡️ CONVERSIONES DEL PROMEDIO:\n"
