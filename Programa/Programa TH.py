...
class SensorMonitorGUI:
    ...
    def setup_ui(self):
        ...
        control_frame = ttk.LabelFrame(self.main_frame, text="Configuración Principal",style="My.TLabelframe", padding="10")
        ...
        unit_frame = ttk.LabelFrame(self.main_frame, text="Unidades de Temperatura",style="My.TLabelframe", padding="10")
        ...
        self.fullscreen_btn = ttk.Button(button_frame, text="⛶ Pantalla Completa", 
                                        command=self.toggle_fullscreen, width=20,cursor="hand2")
        self.fullscreen_btn.grid(row=0, column=6, padx=5)
        
        info_frame = ttk.LabelFrame(self.main_frame, text="Información del Sistema", style="My.TLabelframe", padding="10")
        ...
        values_frame = ttk.LabelFrame(self.main_frame, text="Valores Actuales", style="My.TLabelframe", padding="5")
        ...
        for i in range(5):
            col = i
            row = 0
            sensor_frame = ttk.Frame(values_frame, relief=tk.RIDGE, padding="3")
            sensor_frame.grid(row=row, column=col, padx=2, pady=2, sticky=(tk.W, tk.E, tk.N, tk.S))
            
            header_frame = ttk.Frame(sensor_frame)
            header_frame.grid(row=0, column=0, columnspan=3, sticky=tk.W, pady=(0, 2))
            
            sensor_check = ttk.Checkbutton(header_frame, 
                                          variable=self.selected_sensors[i],
                                          command=self.update_sensor_display)
            sensor_check.grid(row=0, column=0, padx=(0, 2))
            ...
            temp_frame.grid(row=1, column=0, sticky=tk.W, pady=1)
            ttk.Label(temp_frame, text="Temp:").grid(row=0, column=0, sticky=tk.W)
            temp_value_label = ttk.Label(temp_frame, text="--", font=("Arial", 9, "bold"))
            temp_value_label.grid(row=0, column=1, sticky=tk.W, padx=(2, 2))
            temp_symbol_label = ttk.Label(temp_frame, text="°C", font=("Arial", 8))
            temp_symbol_label.grid(row=0, column=2, sticky=tk.W)
            
            hum_frame = ttk.Frame(sensor_frame)
            hum_frame.grid(row=2, column=0, sticky=tk.W, pady=1)
            ttk.Label(hum_frame, text="Hum:").grid(row=0, column=0, sticky=tk.W)
            hum_value_label = ttk.Label(hum_frame, text="--", font=("Arial", 9, "bold"))
            hum_value_label.grid(row=0, column=1, sticky=tk.W, padx=(2, 2))
            ttk.Label(hum_frame, text="%", font=("Arial", 8)).grid(row=0, column=2, sticky=tk.W)
            
            time_frame = ttk.Frame(sensor_frame)
            time_frame.grid(row=3, column=0, sticky=tk.W, pady=(2, 0))
            ttk.Label(time_frame, font=("Arial", 7), foreground="gray").grid(row=0, column=0, sticky=tk.W)
            time_label = ttk.Label(time_frame, text="--", font=("Arial", 7, "bold"), foreground="gray")
            time_label.grid(row=0, column=1, sticky=tk.W, padx=(2, 0))
        ...
        graph_controls_frame = ttk.LabelFrame(self.main_frame, text="Controles de Gráficos", style="My.TLabelframe", padding="10")
        ...
        ttk.Button(toolbar_frame, text="🏠 Restaurar Vista", 
                  command=self.reset_view, width=17).grid(row=0, column=2, padx=(0, 5))
        ttk.Button(toolbar_frame, text="💾 Guardar Gráfico", 
                  command=self.save_figure, width=17).grid(row=0, column=3, padx=(0, 5))
        self.legend_btn = ttk.Button(toolbar_frame, text="Ocultar Leyenda", 
                                      command=self.toggle_legend, width=17)
        self.legend_btn.grid(row=0, column=5, padx=(0, 5))
    ...
    def setup_plots(self):
        ...
        # Colocar el canvas en fila 11
        self.canvas_widget.grid(row=11, column=0, columnspan=4,
                                sticky=(tk.W, tk.E, tk.N, tk.S),
                                padx=10, pady=(0, 9))

        self.root.rowconfigure(11, weight=1)   # El canvas se expande
    ...
    def create_fullscreen_window(self):
        ...
        sensors_frame.pack(side=tk.LEFT, padx=15)
